...
ggplot(psid_hh_size, aes(x = HOUSEHOLD_SIZE, y = pct, fill = size_group)) +
  ...
  labs(
    title = "Distribution of household size, PSID",
...
ggplot(psid %>% filter(INCOME < quantile(INCOME, 0.99)), aes(x = INCOME/1000)) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of household incomes, PSID",
    subtitle = "Binwidth = $5000, outliers above 99th percentile removed", 
...
ggplot() +
  ...
  labs(
    title = "Lorenz Curve of Household Income, adjusted by household heads",
    x = "Cumulative share of households",
    y = "Cumulative share of total income"
  ) +
  theme(legend.position = "bottom",
...
psid_quintile %>%
  ...
  labs(title="Top 20% of households earns larger share of income\nthan bottom 80%", subtitle= "Share of total household income by quintile", x = "Income Quintile", y = "Share of Total Income") +
  theme_minimal() +
  theme(legend.position = "bottom",
        title = element_text(face="bold"))
...
psid_labor_quintile <- psid %>%
  select(TOTAL_LABOR) %>%
  mutate(quintile = ntile(TOTAL_LABOR, 5)) %>%
  group_by(quintile) %>%
...
psid_labor_quintile %>%
  ...
  pander(caption="Share of labor income and mean labor income by quintile")  
...
ggplot() +
  ...
  labs(
    ...
    x = "Cumulative share of households",
    y = "Cumulative share of income/earnings"
  ) +
  theme(legend.position = "bottom",
        title = element_text(face="bold"))
```
...
print(paste("Positive weeks worked and labor earnings:", nrow(psid_positive)))

psid_positive$weekly_wage <- psid_positive$HEAD_LABOR/psid_positive$WEEKS_HEAD
print(paste("Mean weekly wage of household head:", round(mean(psid_positive$weekly_wage),2)))
```
...
tibble(
component = c("Var(log wage)", "Var(log weeks)", "Cov(log wage, log weeks)", "Var(log earnings)"),
  value = c(var_log_wage, var_log_weeks, cov_log, var_log_earnings)
) %>%
  pander("Labor earnings variance decomposition results") 
```
...
```{r wages regression, echo=FALSE, paged.print=TRUE}
...
summary(reg) %>% 
  pander(caption = "Log wages regression results")
```
